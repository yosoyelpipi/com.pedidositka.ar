	...
	if ($sError) {
		...
		if($error){
                    ...
                    for ($i = 0; $i < count($String); $i++) {
                        ...
                        echo json_encode(array("ItsLoginResult" => $ItsPostResult, "contenido" => $Strings, "Cantidad" => $resultados, "idinsertado" => $id, "fecha" => $ItsGetDate, "ws" => $ws, "db" => $db, "user" => $user, "pass" => $pass, "iderr"=>$iderr, "Msg"=> $err, "OutPutJson"=>$salida,"LogOut" => $ItsLogOutResult));
